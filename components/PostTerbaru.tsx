// components/PostTerbaru.tsx - Clickable Cards Version
'use client'
...
export default function PostTerbaru() {
  ...
  const fetchNews = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setNews(sortedData.slice(0, 8)) // Ambil 8 posts terbaru
      } else {
  ...
  const getArticleUrl = (post: News) => {
    return `/berita/umum/${post.id}`
  }

  const handleActionClick = (e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    // Handle action (like, share, comment) without navigating
  }
        ...
        {displayedPosts.length === 0 ? (
          ...
          <div className="space-y-4">
            {displayedPosts.map((post) => (
              ...
              >
                <article className="border-b border-gray-100 last:border-b-0 pb-4 last:pb-0 cursor-pointer hover:bg-gray-50 p-3 -m-3 rounded-lg transition-colors duration-200">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-shrink-0">
                      {post.image_url ? (
                        ...
                        <div className="w-16 h-16 bg-emerald-100 rounded-lg flex items-center justify-center group-hover:bg-emerald-200 transition-colors duration-200">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <h4 className="font-semibold text-gray-900 text-sm leading-snug line-clamp-2 group-hover:text-emerald-700 transition-colors duration-200">
                          {post.title}
                        </h4>
                      ...
                      <div className="mb-3">
                        ...
                        {post.content.length > 100 && (
                          <button
                            ...
                            className="text-emerald-600 text-xs mt-1 hover:text-emerald-700 flex items-center gap-1 z-10 relative"
                          >
                            {expandedPost === post.id ? (
                              <>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-emerald-600 text-xs font-medium flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        ...
                        <div className="flex items-center gap-3 text-gray-400">
                          <button 
                            ...
                            className="hover:text-red-500 transition-colors z-10 relative"
                            title="Suka"
                          >
                          ...
                          <button 
                            onClick={handleActionClick}
                            className="hover:text-blue-500 transition-colors z-10 relative"
                            title="Bagikan"
                          >
                          ...
                          <button 
                            onClick={handleActionClick}
                            className="hover:text-green-500 transition-colors z-10 relative"
                            title="Komentar"
                          >
